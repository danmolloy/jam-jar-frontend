...
function ResetPasswordContent() {
  ...
  const handleRequestReset = async (values: { email: string }) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setMessage(
          "If there's an account linked to that email address, we have sent a password reset link.",
        );
      }
    } catch (err) {
      console.error(err);
  ...
  const handleConfirmReset = async (values: { password: string; password_confirm: string }) => {
    ...
    try {
      ...
      if (!res.ok) {
        const errorData = await res.json();
        throw new Error(
          errorData.password?.[0] || errorData.token?.[0] || 'Failed to reset password',
        );
      } else {
  ...
  return (
    <div className="max-w-md mx-auto mt-8 p-6 bg-white rounded-lg shadow-md font-mono">
      ...
      {message && (
        <div
          className={`mb-4 p-3 rounded ${message.includes('sent') ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}
        >
          {message}
        </div>
      )}
      ...
      >
        {(props) => (
          <Form>
            <InputField label="Email" type="email" name="email" error={props.errors.email} />
            <button
              type="submit"
              disabled={submitting}
