...
export default function WeekBarChart({data, selectedWeek, selectedActivity, selectedTag}: {
  data: {
    day: string
    minutes: number
  }[]
  ...
  <div className={"shadow rounded p-2 bg-white m-2 lg:w-1/2"}>
    <div className='p-2'>
      <h2 className='text-base'>WEEK {selectedWeek.startOf('week').toFormat('dd LLLL yyyy').toUpperCase()}<span className='font-medium'>{selectedActivity !== "" && ` - ${selectedActivity}`}{selectedTag !== "" && ` #${selectedTag}`}</span></h2>
    </div>
